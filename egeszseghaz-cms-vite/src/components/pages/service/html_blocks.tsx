...
export default function ServiceHtmlBlocks({ service }: { service: Service }) {
  ...
  const headingResolved = resolveColor(styles?.htmlBlocks?.headingColor, "text");
  const textResolved = resolveColor(styles?.htmlBlocks?.textColor, "text");

  return (
    <section className={cn("py-16 px-6 sm:px-12 md:px-20", textResolved.className)} style={textResolved.style}>
      <div className="max-w-4xl mx-auto space-y-10">
        {blocks.map((b, i) => (
          <BlurFade key={b.id} delay={0.1 * (i + 1)}>
            <div className={cn("bg-white/70 backdrop-blur-md rounded-xl shadow-md p-6 border", headingResolved.className)} style={headingResolved.style}>
